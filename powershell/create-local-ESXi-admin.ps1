...
foreach ($esxiHost in $hosts) {
    ...
    $cmd = @"
    "$esxiRootUser" -p "$esxiRootPassword" -- "esxcli system account add --id=$user --password=$pass --description='$desc' --role='Admin'"
    "$esxiRootUser" -p "$esxiRootPassword" -- "esxcli system permission set --id=$user --role='Admin'"
"@
    try {
